package com.quootta.mdate.utils.video;
...
@SuppressLint("NewApi")
public class CompressMediaController {
    ...
    private static volatile CompressMediaController Instance = null;
    private boolean videoConvertFirstWrite = true;

    public static CompressMediaController getInstance() {
        CompressMediaController localInstance = Instance;
        if (localInstance == null) {
            synchronized (CompressMediaController.class) {
                localInstance = Instance;
                if (localInstance == null) {
                    Instance = localInstance = new CompressMediaController();
                }
            }
        }
    ...
    public static class VideoConvertRunnable implements Runnable {
        ...
        @Override
        public void run() {
            CompressMediaController.getInstance().convertVideo(videoPath);
        }
    }
    ...
    public boolean convertVideo(final String path) {
        ...
        File cacheFile = new File(
                Environment.getExternalStorageDirectory()
                        + File.separator
                        + LocalConfig.FILE_DIR
                        + LocalConfig.FILE_COMPRESSED_VIDEOS_DIR,
//                "VIDEO_" + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(new Date()) + ".mp4"
                LocalConfig.VIDEO_NAME
        );
