...
public class MeFollowFragment extends BaseFragment {
    ...
    @Bind(R.id.rl_me_fallow)
    RecyclerView rl_me_fallow;
    @Override
    ...
    public void onEventMes(FavoriteList favoriteList) {
        ...
        userList.users=favoriteList.getData().getUsers().getTo_me();
    ...
    protected void setListener() {
        //上拉加载时，从当前页数继续向下请求
        rl_me_fallow.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void requestFavoriteList() {
        ...
        FriendsListRequest favoriteListRequest = new FriendsListRequest(paramsMap, new VolleyListener() {
            ...
            protected void onSuccess(JSONObject response) {
                ...
                if (list.getData().getUsers().getTo_me().size()==0){
                    ToastUtil.showToast(getString(R.string.app_tips_text15));
                }else {
                    userList.users.addAll(list.getData().getUsers().getTo_me());
    ...
    private void initRecyclerView(final UserList userList) {
        ...
        rl_me_fallow.setAdapter(rvFriendsAdapter);

        linearLayoutManager = new LinearLayoutManager(getContext(),LinearLayoutManager.VERTICAL,false);
        rl_me_fallow.setLayoutManager(linearLayoutManager);
