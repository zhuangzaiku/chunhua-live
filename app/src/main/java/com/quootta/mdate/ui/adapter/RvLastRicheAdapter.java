 ...
 */
public class RvLastRicheAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>   {


    private Context context;
   // private List<RankingListData.DataBean.ThisWeekBean.RicheBoardBean.RichesBean> ThisWeek_Riches;
       private List<RankingListData.DataBean.LastWeekBean.RicheBoardBean.RichesBean> LastWeek_Riches;
...
//    private List<RankingListData.DataBean.LastWeekBean.CharmBoardBean.CharmsBean> LastWeek_Charms;
    ...
    public RvLastRicheAdapter (Context context, List<RankingListData.DataBean.LastWeekBean.RicheBoardBean.RichesBean> LastWeek_Riche) {
        this.context = context;
        this.LastWeek_Riches = LastWeek_Riche;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof RichBoardViewHolder) {
            ...
            position = position + 2;
            //土豪值
         //   ((RichBoardViewHolder) holder).rankingItemDiademNum.setText(LastWeek_Riches.get(position).getRiche_score().split("\\.")[0]);
            //昵称
            ((RichBoardViewHolder) holder).rankingItemName.setText(LastWeek_Riches.get(position).getNick_name());
            //年龄
            ((RichBoardViewHolder) holder).rankingItemAge.setText(LastWeek_Riches.get(position).getAge() + "");

            //判断性别
            if (LastWeek_Riches.get(position).getGender().equals("male")) {
            ...
            //排名
            ((RichBoardViewHolder) holder).rankingItemNum.setText(LastWeek_Riches.get(position).getRiche_range() + 1 + "");
            //加载图片
            imageLoader.get(LocalUrl.getPicUrl(LastWeek_Riches.get(position).getAvatar()),
            ...
            //第一名
            if (LastWeek_Riches.size() >= 1) {
                //昵称
                ((TopItemViewHolder) holder).rankingName1.setText(LastWeek_Riches.get(0).getNick_name());
                //添加年龄数据
                if (LastWeek_Riches.get(0).getAge() == -1) {
                    ((TopItemViewHolder) holder).rankingAge1.setText("18");
                } else {
                    ((TopItemViewHolder) holder).rankingAge1.setText(LastWeek_Riches.get(0).getAge() + "");
                }

                //判断性别
                if (LastWeek_Riches.get(0).getGender().equals("male")) {
                ...
                //土豪值
               // ((TopItemViewHolder) holder).richeScore1.setText(LastWeek_Riches.get(0).getRiche_score().split("\\.")[0]);

                //添加图片
                imageLoader.get(LocalUrl.getPicUrl(LastWeek_Riches.get(0).getAvatar()),
                ...
                //第二名
                if (LastWeek_Riches.size() >= 2) {

                    //添加年龄数据
                    if (LastWeek_Riches.get(1).getAge() == -1) {
                        ((TopItemViewHolder) holder).rankingAge2.setText("18");
                    } else {
                        ((TopItemViewHolder) holder).rankingAge2.setText(LastWeek_Riches.get(1).getAge() + "");
                    }
                    //判断性别
                    if (LastWeek_Riches.get(1).getGender().equals("male")) {
                    ...
                    //昵称
                    ((TopItemViewHolder) holder).rankingName2.setText(LastWeek_Riches.get(1).getNick_name());

                    //土豪值
                 //   ((TopItemViewHolder) holder).richeScore2.setText(LastWeek_Riches.get(1).getRiche_score().split("\\.")[0]);

                    //添加图片
                    imageLoader.get(LocalUrl.getPicUrl(LastWeek_Riches.get(1).getAvatar()),
                    ...
                    //第三名
                    if (LastWeek_Riches.size() >= 3) {

                        //添加年龄数据
                        if (LastWeek_Riches.get(2).getAge() == -1) {
                            ((TopItemViewHolder) holder).rankingAge3.setText("18");
                        } else {
                            ((TopItemViewHolder) holder).rankingAge3.setText(LastWeek_Riches.get(2).getAge() + "");
                        }

                        //判断性别
                        if (LastWeek_Riches.get(2).getGender().equals("male")) {
                        ...
                        //昵称
                        ((TopItemViewHolder) holder).rankingName3.setText(LastWeek_Riches.get(2).getNick_name());

                        //土豪值
                  //      ((TopItemViewHolder) holder).richeScore3.setText(LastWeek_Riches.get(2).getRiche_score().split("\\.")[0]);
                        //添加图片
                        imageLoader.get(LocalUrl.getPicUrl(LastWeek_Riches.get(2).getAvatar()),
    ...
    public int getItemCount() {
        if (LastWeek_Riches.size()>=100){
            return 98;//100-2
        }else {

            return LastWeek_Riches.size() - 2;
        }
    }
        ...
        public TopItemViewHolder(View itemView) {
            ...
            rankingOne.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("user_id", LastWeek_Riches.get(0).get_id());
            ...
            rankingTwo.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("user_id", LastWeek_Riches.get(1).get_id());
                    context.startActivity(intent);
                }
            });
            //单击第三个item
            rankingThree.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("user_id", LastWeek_Riches.get(2).get_id());
                    context.startActivity(intent);
                }
            });
        ...
        public RichBoardViewHolder(View itemView) {
            ...
            rankingItem.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("user_id", LastWeek_Riches.get(getLayoutPosition() + 2).get_id());
                    context.startActivity(intent);
                }
            });
        }
