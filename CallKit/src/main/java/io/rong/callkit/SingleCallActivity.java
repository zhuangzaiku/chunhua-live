...
public class SingleCallActivity extends BaseCallActivity implements Handler.Callback {
    ...
    //视频赠送礼物
    private static onGiftClick onGiftClick;
    public interface onGiftClick{
        void onSendGift(Context context, String id);
    }

    public static void setOnGiftClick(onGiftClick onGiftClick){
        SingleCallActivity.onGiftClick=onGiftClick;
    }
...
//        }
    ...
    public void onCallConnected(RongCallSession callSession, SurfaceView localVideo) {
        ...
        //美颜按钮
        meiyanimage= (ImageView) findViewById(R.id.rc_voip_call_meiyan);


        if (callSession.getMediaType().equals(RongCallCommon.CallMediaType.AUDIO)) {
            //语音界面 切换到minimize界面 使其不可见
            findViewById(R.id.rc_voip_call_minimize).setVisibility(View.INVISIBLE);
            ...
            //开启美颜
            yuvEnhancer=new AgoraYuvEnhancer(getApplication());
            yuvEnhancer.StartPreProcess();
            isBeautiful=false;
            meiyanimage.setImageResource(R.drawable.meiyan_on);
        }
    ...
    public void hideVideoCallInformation() {
        ...
        //隐藏礼物和美颜按钮
        findViewById(R.id.rc_voip_call_meiyan).setVisibility(View.GONE);
        findViewById(R.id.rc_voip_audio_gift).setVisibility(View.GONE);
    }

    public void showVideoCallInformation() {
        isInformationShow = true;
        mUserInfoContainer.setVisibility(View.VISIBLE);
        //视频通话界面 切换到迷你界面 使其不可见
        mUserInfoContainer.findViewById(R.id.rc_voip_call_minimize).setVisibility(View.GONE);
        //美颜开关设为可见
        findViewById(R.id.rc_voip_call_meiyan).setVisibility(View.VISIBLE);
...
//        view.setVisibility(View.VISIBLE);
        ...
        //发送礼物
        View giftView=findViewById(R.id.rc_voip_audio_gift);
        giftView.setVisibility(View.VISIBLE);
        giftView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                onGiftClick.onSendGift(SingleCallActivity.this,targetId);
            }
        });
    }
...
//
//    //开启关闭美颜
    public void onMeiyanClick(View view){
        Toast meiyanToast;
        if (isBeautiful){
            // 开启美颜
            //美颜
            ...
            isBeautiful=false;
            meiyanimage.setImageResource(R.drawable.meiyan_on);
            meiyanToast= makeText(getBaseContext(),"打开美颜",Toast.LENGTH_SHORT);
            meiyanToast.setGravity(Gravity.CENTER|Gravity.CENTER_HORIZONTAL,0,0);
            meiyanToast.show();
        }else {
            //关闭美颜
            if (yuvEnhancer!=null){
                ...
                yuvEnhancer.StopPreProcess();
                isBeautiful=true;
                meiyanimage.setImageResource(R.drawable.meiyan_off);
                meiyanToast=makeText(getBaseContext(),"关闭美颜",Toast.LENGTH_SHORT);
                meiyanToast.setGravity(Gravity.CENTER|Gravity.CENTER_HORIZONTAL,0,0);
                meiyanToast.show();
            }
        }
